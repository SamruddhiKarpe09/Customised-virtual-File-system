...
void DisplayHelp()
{
    ...
    printf("exit : To Terminate the filesystem\n");
...
int ReadFile(int fd, char *arr, int isize)
{
    ...
    else
    {
        ...
        (UFDTArr[fd].ptrfiletable->readoffset) = (UFDTArr[fd].ptrfiletable->readoffset) = (UFDTArr[fd].ptrfiletable->readoffset) + isize;
...
int LseekFile(int fd, int size,int from)
{
    ...
    else if(UFDTArr[fd].ptrfiletable->mode == WRITE)
    {
        ...
        {
            if((UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) + size > MAXFILESIZE)      return -1;
            if(((UFDTArr[fd].ptrfiletable->writeoffset) = (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) + size) < 0)    return -1;
...
void ls_file()
{
    ...
    printf("\nFile Name\tInode number\tfile size\tLunk count\n");
    printf("---------------------------------------------------------\n");
    while(temp != NULL)
    {
        if(temp->FileType != 0)
        {
            printf("%s\t\t%d\t\t%d\n",temp->FileActualSize,temp->InodeNumber,temp->LinkCount);
        }
        temp = temp->next;
    }
    printf("----------------------------------------------------------\n");
    ...
    while(1)
    {
        ...
        else if(count == 3)
        {
            ...
            else if(strcmp(command[0],"read") == 0)
            {
                ...
                if(ptr == NULL)
                {
                    printf("ERROE :Memory allocation failure\n");
                    continue;
                }
